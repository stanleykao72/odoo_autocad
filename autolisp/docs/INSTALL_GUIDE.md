...
| 順序 | 檔案 | 說明 |
|------|------|------|
| 1 | `json_util.lsp` | JSON 解析器（純 AutoLISP） |
| 2 | `file_util.lsp` | 檔案 I/O 工具 |
| 3 | `config.lsp` | 路徑與設定管理 |
| 4 | `strip_mtext.lsp` | MText 格式清理 |
| 5 | `block_util.lsp` | Block / Attribute 操作 |
| 6 | `table_util.lsp` | TABLE 實體操作 |
| 7 | `ob_mcp_dispatch.lsp` | Odoo 擴展指令（6 個 action） |
| 8 | `main.lsp` | **入口點** — 載入上述模組，註冊 `OB:MCP-DISPATCH` |
...
APPLOAD
```
瀏覽到 `C:\odoo\autocad_source\autolisp\lisp\main.lsp`，點 **Load**。
...
[OB] mcp_dispatch.lsp loaded from autocad-mcp submodule
...
在 APPLOAD 對話框中，將 `main.lsp` 加入 **Startup Suite**。
...
| `OdooAutoCAD.vlx` | 8 個 Odoo .lsp（見 Section 2.1） | Odoo 整合模組 |
...
2. 加入源碼（順序重要 — 依賴關係）：
   ...
   7. lisp/ob_mcp_dispatch.lsp
   8. lisp/main.lsp              ← Entry Point
   ```
3. Project → Build Project
...
因為 `ob_mcp_dispatch.lsp` 會引用 `mcp-dispatch-command` 函數。
...
```lisp
;; build_vlx.lsp — 位於 autolisp/build_vlx.lsp
;; 使用方式: (load "build_vlx.lsp") 然後 BUILD-ALL

;;; === Helper: compile .lsp → .fas ===
(defun ob:compile-one (src-path / fas-path)
...
(defun c:BUILD-MCP-DISPATCH ( / root fas-list)
  (setq root "C:/odoo/autocad_source/libs/autocad-mcp/lisp-code/")
  (setq fas-list (list
    (ob:compile-one (strcat root "attribute_tools.lsp"))
    (ob:compile-one (strcat root "mcp_dispatch.lsp"))
  ))
  ;; Make VLX
  (command "_.VLISP-MAKE-APP"
    "McpDispatch"
    "C:/odoo/autocad_source/autolisp/dist/McpDispatch.vlx"
    (nth 0 fas-list)
    (nth 1 fas-list)
    ""
  )
  (princ "\n[BUILD] McpDispatch.vlx done")
  (princ)
)

;;; === 2. Build OdooAutoCAD.vlx ===
(defun c:BUILD-ODOO-VLX ( / root fas-list)
  (setq root "C:/odoo/autocad_source/autolisp/lisp/")
  (setq fas-list (list
    (ob:compile-one (strcat root "json_util.lsp"))
    (ob:compile-one (strcat root "file_util.lsp"))
    (ob:compile-one (strcat root "config.lsp"))
    (ob:compile-one (strcat root "strip_mtext.lsp"))
    (ob:compile-one (strcat root "block_util.lsp"))
    (ob:compile-one (strcat root "table_util.lsp"))
    (ob:compile-one (strcat root "ob_mcp_dispatch.lsp"))
    (ob:compile-one (strcat root "main.lsp"))
  ))
  ;; Make VLX
  (command "_.VLISP-MAKE-APP"
    "OdooAutoCAD"
    "C:/odoo/autocad_source/autolisp/dist/OdooAutoCAD.vlx"
...
(defun c:BUILD-ALL ()
  ...
  (princ "\n[BUILD]   dist/McpDispatch.vlx")
  (princ "\n[BUILD]   dist/OdooAutoCAD.vlx")
  (princ "\n[BUILD] ========================================")
  (princ)
)
```
...
```
[OB] Warning: Could not find json_util.lsp
```
