...
class UtilPushToBoq:
    ...
    def push_to_boq(self, progress_callback=None):
        """推送到 BOQ — 統一流程（COM/IPC 皆使用相同路徑）

        嘗試 per-layout 提取 → 若 get_doc_layouts() 回傳空則 fallback 批次提取。
        兩端 get_layouts_values() 都回傳 {"all": [...]}, 不再需要模式判斷。
        ...
        # Step 1: Try per-layout extraction
        ...
        if not layout_dic:
            # Fallback: single bulk call (e.g. get_doc_layouts unsupported or empty)
            _progress(0.10, "提取所有 layout 資料（批次）...")
            _log("[BOQ] 呼叫 get_layouts_values() (批次)...\n")
            result = self.autocad_util.get_layouts_values()
            # Both backends now return {"all": [...]}
            if isinstance(result, dict) and "all" in result:
                layout_dic = result["all"]
            elif isinstance(result, list):
                layout_dic = result
            else:
                layout_dic = []
            _log(f"[BOQ] get_layouts_values() 回傳 {len(layout_dic)} 筆\n")
            _progress(0.70, "提取完成")
        ...
        # Step 3: Push to Odoo — always wrap in {"all": [...]}
        layout_payload = {"all": layout_dic} if isinstance(layout_dic, list) else layout_dic
        _progress(0.75, "推送資料到 Odoo...")
        _log(f"[BOQ] 呼叫 import2boq(), keys={list(layout_payload.keys())}\n")
        boq_list = self.odoo_util.import2boq(layout_payload)
