...
### How to verify
- [ ] ProgId field renders as read-only showing "AutoCAD.Application" (AC-01)
- [ ] Connection Timeout field renders with default value 10 (AC-02)
- [ ] Retry Attempts field renders with default value 3 (AC-03)
...
### How to verify
- [ ] Properties generate change notifications and are bindable (AC-01, AC-02, AC-03)
- [ ] Modifying timeout or retry attempts sets `HasUnsavedChanges = true` (AC-06 context)
...
### How to verify
- [ ] After Save, `ServerConfigs` contains rows for `autocad.prog_id`, `autocad.connection_timeout`, `autocad.retry_attempts` (AC-06)
- [ ] `SyncLog` entry includes AutoCAD setting changes (AC-08)
...
### How to verify
- [ ] On page load, fields show values from DB if present (AC-07)
- [ ] On first load (no DB entries), fields show defaults: ProgId="AutoCAD.Application", Timeout=10, Retry=3 (AC-07)
